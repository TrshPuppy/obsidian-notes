...
INIT.
OWE was introduced alongside [SAE](WPA3.md)/WPA3 and offers an alternative for networks that were previously completely open (OPN networks). In OWE, user traffic *is [encrypted](../../OSCP/password-attacks/README.md)* without requiring a shared password.
...
Essentially, the same attacks which threaten OPN networks can be used against OWE networks, just with the caveat that *user traffic is encrypted and can't be viewed*.
