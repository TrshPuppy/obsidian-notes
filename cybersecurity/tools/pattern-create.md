...
### `-l` (length)
The `-l` flag is the length we want the string that `pattern_create` makes for us to be. The number we give is *based on the offset we found* using [spiking](/nested-repos/PNPT-study-guide/PEH/buffer-overflows/spiking.md) and [fuzzing](/nested-repos/PNPT-study-guide/PEH/buffer-overflows/fuzzing.md).
